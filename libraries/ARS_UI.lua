...
for id, asset in next,assets do
  local path = "informant_cache/asset_" .. tostring(id) .. ".png"
  if not isfolder("informant_cache") then
    makefolder("informant_cache")
  end
  ...
  local _base_0 = {
    ...
    apply_settings = function(self, tab)
      ...
      config_group:new_button({
        text = "Save",
        callback = function()
          local config_name = "informant/" .. tostring(getgenv().window.flags["config_name"]) .. ".json"
          if (not isfolder("informant")) then
            makefolder("informant")
          end
          local fixed_config = { }
